...
// Wersja builda — busting pamięci podręcznej WebView/przeglądarki dla modułów danych
...
// Manifest z wersjonowanymi ścieżkami — app.js importuje moduły z ?v=VERSION,
// więc aktualizacja danych nie może zagrać z pamięcią podręczną WebView
const manifest =
  "// AUTO-GENERATED by scripts/build_materials.js — nie edytuj ręcznie.\n" +
  "export const VERSION = " + JSON.stringify(VERSION) + ";\n" +
  "export const MODULES = " + JSON.stringify(
    Object.fromEntries(MATERIALS.map(m => [m.id, "./data/material-" + m.id + ".js?v=" + VERSION]))
  ) + ";\n";
const manifestFile = path.join(OUT_DIR, "materials-manifest.js");
fs.writeFileSync(manifestFile, manifest, "utf8");
console.log("[build:materials] manifest -> " + path.relative(ROOT, manifestFile) + " (v=" + VERSION + ")");
