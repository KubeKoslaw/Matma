#!/usr/bin/env node
// build_materials.js - Konwertuje materiały markdown na dane strukturalne:
//   teoria  -> karty (podział części teoretycznej na H2)
//   zadania -> obiekty {id, group, flaggedR, question, answer, solution}
// Wyjście: src/data/generated/material-*.json (typowane przez src/data/types.ts;
//          lazy-import w src/dzialy.ts przez import.meta.glob).
...
const LABEL_RE =
  /^\*\*\s*(Odpowiedź i Rozwiązanie krok po kroku|Rozwiązanie krok po kroku|Odpowiedź|Treść zadania|Treść)\s*:?\s*\*\*.*$/;
...
function parseTasks(text, groupId, out, usedIds) {
  ...
  marks.forEach((mk, i) => {
    ...
    const tresc = take("Treść zadania") ?? take("Treść");
    const answer = combined === null ? take("Odpowiedź") : null;
    const solution = combined ?? take("Rozwiązanie krok po kroku");
    // Fallback pytania dopiero PO wycięciu odpowiedzi/rozwiązania z reszty
    const question = tresc ?? (stripTrailingHr(rest).trim() || "");
...
for (const m of MATERIALS) {
  ...
  const outFile = path.resolve(OUT_DIR, "material-" + m.id + ".json");
  if (!outFile.startsWith(OUT_DIR + path.sep)) {
    console.error("[build:materials] Ścieżka wyjściowa poza katalogiem: " + outFile);
    process.exitCode = 1;
    continue;
  }

  // Czysty JSON — cache-busting zapewniają haszowane nazwy chunków Vite
  fs.writeFileSync(outFile, JSON.stringify(parsed), "utf8");
  console.log("[build:materials] " + m.id + ": " + (JSON.stringify(parsed).length / 1024).toFixed(0) + " kB -> " + path.relative(ROOT, outFile));
}
